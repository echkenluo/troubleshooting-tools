...
// kvm_irqfd_stats_summary_arm - VM interrupt statistics for ARM GIC BPF program
...
/* VGIC queue histogram */
struct {
    __uint(type, BPF_MAP_TYPE_LRU_HASH);
    __uint(max_entries, 10240);
    __type(key, struct vgic_queue_hist_key);
    __type(value, __u64);
} vgic_queue_hist SEC(".maps");
...
/* VGIC populate LR histogram */
struct {
    __uint(type, BPF_MAP_TYPE_LRU_HASH);
    __uint(max_entries, 10240);
    __type(key, struct vgic_populate_lr_hist_key);
    __type(value, __u64);
} vgic_populate_lr_hist SEC(".maps");
...
/* Trace: vgic_queue_irq_unlock (ARM-specific) */
SEC("kprobe/vgic_queue_irq_unlock")
int BPF_KPROBE(kprobe_vgic_queue_irq_unlock, struct kvm *kvm, void *irq)
{
    __u32 intid = 0;
    __u8 *is_active;
    __u64 kvm_ptr_key;
    struct vgic_queue_hist_key hist_key = {};

    if (!kvm || !irq || (__u64)kvm < 0xffff000000000000ULL ||
        (__u64)irq < 0xffff000000000000ULL)
        return 0;
    ...
    bpf_probe_read_kernel(&intid, sizeof(intid), irq);

    hist_key.kvm_ptr = (__u64)kvm;
    hist_key.intid = intid;
    hist_key.slot = 0;
    hist_increment(&vgic_queue_hist, &hist_key);
...
/* Trace: vgic_v2_populate_lr (ARM GICv2 specific) */
SEC("kprobe/vgic_v2_populate_lr")
int BPF_KPROBE(kprobe_vgic_v2_populate_lr, struct kvm_vcpu *vcpu, void *irq)
{
    struct kvm *kvm = NULL;
    __u32 intid = 0;
    __u8 *is_active;
    __u64 kvm_ptr_key;
    struct vgic_populate_lr_hist_key hist_key = {};

    if (!vcpu || !irq || (__u64)vcpu < 0xffff000000000000ULL ||
        (__u64)irq < 0xffff000000000000ULL)
        return 0;
    ...
    hist_key.version = 2;
    hist_key.slot = 0;
    hist_increment(&vgic_populate_lr_hist, &hist_key);

    return 0;
}

/* Trace: vgic_v3_populate_lr (ARM GICv3 specific) */
SEC("kprobe/vgic_v3_populate_lr")
int BPF_KPROBE(kprobe_vgic_v3_populate_lr, struct kvm_vcpu *vcpu, void *irq)
{
    struct kvm *kvm = NULL;
    __u32 intid = 0;
    __u8 *is_active;
