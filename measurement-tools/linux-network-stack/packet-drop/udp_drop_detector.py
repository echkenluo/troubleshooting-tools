...
struct udp_group_key {
    ...
    u8 direction;         // 0=forward, 1=reverse (reserved)
    u8 pad;
};
...
struct event_t {
    ...
    u8 is_fragmented:1;   // Is this a fragmented datagram?
    u8 reserved:5;
...
// Parse UDP packet and fill group key
// Returns: 1=matched forward direction, 0=not matched
static __always_inline int parse_udp_packet(struct sk_buff *skb,
    struct udp_group_key *key, struct event_t *evt)
{
    unsigned char *head;
    ...
    // For reverse direction, user should run another instance with swapped IPs
    if (actual_sip != SRC_IP_FILTER || actual_dip != DST_IP_FILTER)
        return 0;

    // Extract IP ID and fragment info
    key->sip = actual_sip;
    key->dip = actual_dip;
    key->ip_id = ip.id;
    key->direction = 0;
    ...
    u16 frag_offset = (frag_off_raw & IP_OFFSET_MASK) * 8;  // Convert to bytes

    evt->is_fragmented = (more_frag || frag_offset) ? 1 : 0;

    // Track first fragment (offset=0)
    if (frag_offset == 0) {
        evt->has_first_frag = 1;

        // First fragment has UDP header - extract ports
        ...
        // Non-first fragment - no UDP header
        evt->sport = 0;
        evt->dport = 0;

        // Skip port filter for non-first fragments (we track by IP ID)
        // Port filtering will be done in userspace based on group state
    }

    // Track last fragment (MF=0 and either fragmented or standalone)
    if (!more_frag) {
        evt->has_last_frag = 1;
    }

    // Calculate payload for this fragment/packet
    u16 ip_len = ntohs(ip.tot_len);
    u8 ip_ihl = ip.ihl & 0x0F;
    u16 ip_hdr_len = ip_ihl * 4;
    u16 payload = 0;
    if (ip_len > ip_hdr_len) {
        payload = ip_len - ip_hdr_len;
        if (frag_offset == 0) {
            // First fragment: subtract UDP header
            payload = (payload > 8) ? (payload - 8) : 0;
        }
    }
...
TRACEPOINT_PROBE(net, netif_receive_skb) {
    ...
    if (!parse_udp_packet(skb, &key, &evt))
        return 0;
    ...
    if (evt.has_first_frag) {
        group->has_first_frag = 1;
        if (evt.sport != 0) {
            group->sport = evt.sport;
            group->dport = evt.dport;
        }
    }
    if (evt.has_last_frag) {
        group->has_last_frag = 1;
    }
    if (evt.is_fragmented) {
        group->is_fragmented = 1;
    }
    group->total_payload += evt.total_payload;

    bpf_probe_read_kernel_str(group->ifname, sizeof(group->ifname), dev->name);
...
int kprobe__dev_queue_xmit(struct pt_regs *ctx, struct sk_buff *skb) {
    ...
    // Clean up completed groups (both stages seen and fragment complete)
...
class Event(ctypes.Structure):
    _fields_ = [
        ...
        ("flags", ctypes.c_uint8),  # has_first_frag, has_last_frag, is_fragmented
    ...
    @property
    def has_first_frag(self):
        return bool(self.flags & 0x01)

    @property
    def has_last_frag(self):
...
class GroupTracker:
    ...
    def _report_drop(self, key, group, drop_type):
        ...
        port_info = ""
        if group["sport"] and group["dport"]:
