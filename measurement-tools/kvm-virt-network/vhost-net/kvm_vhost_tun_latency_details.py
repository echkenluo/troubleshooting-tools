...
Phase 2 (measure):
  - Use a Phase 1 profile to filter and emit per-packet latency (exact values).
...
Examples:
  ...
  sudo %(prog)s --mode measure --profile profile.json --duration 30
  # Disable per-packet detail output
  sudo %(prog)s --mode measure --profile profile.json --duration 30 --no-detail

Notes:
  - Requires root, BCC, and kernel headers.
  - Assumes tfile->napi_enabled is off and RPS is disabled.
  - Emits per-packet latency samples for the target flow (no histogram approximation).
  - Default prints every per-packet event; use --no-detail to suppress.
"""
...
RAW_TRACEPOINT_PROBE(netif_receive_skb) {
    ...
    // Emit per-packet event only for flow-matched chain
    u8 s0_ok_val = 0;
    u8 s1_ok_val = 0;
    u64 s0_us = 0;
    u64 s1_us = 0;
    ...
    // Queue mapping (for multi-queue TUN)
    u16 qmap = 0;
    bpf_probe_read_kernel(&qmap, sizeof(qmap), &skb->queue_mapping);
...
def fmt_avg(sum_us, count):
    return (sum_us / float(count)) if count else None
...
def run_discover(args):
    ...
    if args.duration:
        ...
        print("Discover mode: running until Ctrl+C")
        try:
            while True:
                time.sleep(1)
...
def run_measure(args):
    ...
    if args.duration:
        print("\nMeasure mode: duration={}s".format(args.duration))
    else:
        print("\nMeasure mode: running until Ctrl+C")
    ...
    print("Total samples: S0={} S1={} S2={} chain(all)= {}".format(
        total_s0_cnt, total_s1_cnt, total_s2_cnt, total_chain_cnt))
    ...
    s0_avg = fmt_avg(total_s0_sum, total_s0_cnt)
    s1_avg = fmt_avg(total_s1_sum, total_s1_cnt)
    s2_avg = fmt_avg(total_s2_sum, total_s2_cnt)
    chain_avg = fmt_avg(total_chain_sum, total_chain_cnt)

    print("\nExact averages (us):")
    print("  S0 avg: {}".format("{:.3f}".format(s0_avg) if s0_avg is not None else "n/a"))
    print("  S1 avg: {}".format("{:.3f}".format(s1_avg) if s1_avg is not None else "n/a"))
    print("  S2 avg: {}".format("{:.3f}".format(s2_avg) if s2_avg is not None else "n/a"))
    print("  S0+S1+S2 avg (per-packet): {}".format(
        "{:.3f}".format(chain_avg) if chain_avg is not None else "n/a"))
    print()


def main():
    parser = argparse.ArgumentParser(
        description="KVM -> vhost -> tun latency tool (details, two-phase)",
    ...
    parser.add_argument("--no-detail", action="store_true", help="Disable per-packet detail output")
